...
public class ElGamal {
    ...
    try {
        ...
        br.close();
        BigInteger m = recoverSecret(p, g, y, year, month, day, hour, minute, second, c1, c2);
    ...
    int second, BigInteger c1, BigInteger c2) {
        int millis = 0;
        for(;;) {
            BigInteger randomNumber = new BigInteger("0");
            int[] pow = {10, 8, 6, 4, 2, 0, 0};
            int[] time = {year, month, day, hour, minute, second, millis};
            for (int j = 0; j < 7; j++)
                randomNumber = randomNumber.add(BigInteger.valueOf(time[j]).multiply(BigInteger.valueOf(10).pow(pow[j])));
            if (c1.equals(g.modPow(randomNumber, p)))
                return c2.multiply(y.modInverse(p).modPow(randomNumber, p)).mod(p);
            millis ++;
        }
    }
}
