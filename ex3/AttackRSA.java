...
public class AttackRSA {
	...
	private static BigInteger recoverMessage(BigInteger[] N, BigInteger[] e, BigInteger[] c) {
		BigInteger N1N2N3 = N[0].multiply(N[1]).multiply(N[2]);
		BigInteger C = BigInteger.ZERO;
		BigInteger niHat, vi;

		for (int i = 0; i < 3; i++) {
			niHat = N1N2N3.divide(N[i]);
			vi = niHat.modInverse(N[i]);
			C = C.add(c[i].multiply(niHat).multiply(vi));
		}

		return CubeRoot.cbrt(C.mod(N1N2N3));
	}
